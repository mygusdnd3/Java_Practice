...
public class Test06_01 {
	...
	public void setOne() {
		...
		// 객체에서 변수에 접근
		System.out.printf("이름  :%3s\n전화번호 :%13s\n이메일  :%-25s\n나이 : %3d\n성별 :%2s", p1.name, p1.tel, p1.mail,
				p1.age, p1.gen==('M') ? "남자" : "여자") ; 
...
//				p1.mail, p1.age, p1.gen);
	...
	public void setMany() {
		setArray(); // 		*****선언안해주면 하단에 Nul Exception 발생
		// 데이터 배열 초기화
	...
	public void toPrint() {			
		for(int i = 0 ; i<friend.length;i++ ) {		//전역에 friend 레퍼런스가 없으면 길이못써..
			...
			System.out.printf("이름  :%3s\n전화번호 :%13s\n이메일  :%-25s\n나이 : %3d\n성별 :%2s", friend[i].name, friend[i].tel, friend[i].mail,
					friend[i].age, friend[i].gen==('M') ? "남자" : "여자") ; 
